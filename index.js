...
videos.forEach((v, i) => {
  ...
  vi.muted = "true";
  vi.loop = "true";
  const top = (i % 4) < 2 ? 0 : (vi.height) % 800;
  const left = i % 2 == 0 ? 0 : vi.width ;
  vi.style=`position:absolute;top: ${top}px;left: ${left}px`;
  if(i < start || i > end) {
    vi.style.opacity = 0 ;
  }
});
...
setInterval(() => {
  ...
  document.querySelectorAll('video').forEach((vi,i) => {
    if(i < start || i > end) {
    vi.style.opacity = 0 ;
   } else {
     vi.style.opacity = 1 ;
